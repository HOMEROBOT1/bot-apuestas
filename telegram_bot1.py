...
# =========================================================
# CONFIG INTELIGENTE (Railway + Backup)
# =========================================================
...
print("=== CONFIG DEBUG ===")
print("BOT_TOKEN:", "OK" if BOT_TOKEN and ":" in BOT_TOKEN else "ERROR")
print("CHAT_ID:", CHAT_ID)
print("USANDO RAILWAY:", os.getenv("BOT_TOKEN") is not None)
print("====================")
...
STRATEGY_MODE = get_env("STRATEGY_MODE", "balanceado").strip().lower()
...
OVER15_MAX_MIN_MINUTE = 70

OVER25_MIN_MINUTE = 25
OVER25_MAX_MIN_MINUTE = 75

UNDER35_MIN_MINUTE = 20
UNDER35_MAX_MIN_MINUTE = 78

NEXT_GOAL_MIN_MINUTE = 18
NEXT_GOAL_MAX_MIN_MINUTE = 80
...
def is_good_under35_state(minute: int, total_goals: int, shots_total: int, corners_total: int) -> bool:
    return (
        UNDER35_MIN_MINUTE <= minute <= UNDER35_MAX_MIN_MINUTE
        and total_goals <= 2
        and shots_total <= 6
        and corners_total <= 9
    )
...
def should_signal_over15(minute: int, total_goals: int, shots_total: int, corners_total: int) -> bool:
    return (
        OVER15_MIN_MINUTE <= minute <= OVER15_MAX_MIN_MINUTE
        and total_goals <= 1
...
def should_signal_over25(minute: int, total_goals: int, shots_total: int, corners_total: int) -> bool:
    return (
        OVER25_MIN_MINUTE <= minute <= OVER25_MAX_MIN_MINUTE
        and total_goals <= 2
...
def should_signal_next_goal(minute: int, shots_total: int, corners_total: int, attack_diff: int) -> bool:
    return (
        NEXT_GOAL_MIN_MINUTE <= minute <= NEXT_GOAL_MAX_MIN_MINUTE
...
async def startup_message():
    msg = (
        f"🤖 Bot de apuestas V9 unido iniciado\n\n"
        f"Funciones activas:\n"
        f"• Config inteligente Railway + respaldo en código\n"
